...
`#[derive(Dialect)]` auto-generates `HasDigraphs`/`HasUngraphs` impls for all dialects. Dialects with no `DiGraph`/`UnGraph` fields get empty-iterator impls (same pattern as `HasBlocks`/`HasCfgs`).
