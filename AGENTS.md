...
**Interpreter:**
- `kirin-interpreter` — interpreter framework. Shared pieces: `Interp`, `InterpretCtx`, `Interpretable<C>`, `Frame`, and `drive_frames`. Forward pieces: `ValueContext`, `Env`, `ForwardEffect`, `ConcreteInterpreter`, and `ForwardAbstractInterpreter`. `AbstractInterpreter` is the marker trait for lattice-valued engines.
...
- **`Interp` is the engine driver; `Interpretable<C>` is the dialect trait**: `Interp` exposes `Value`, `Error`, `Effect`, and `Context<'a>`. Dialect rules specialize on a context type, not directly on an engine type. Forward rules use `Interpretable<ValueContext<'_, I>>`; future analyses should define their own context type instead of adding more cases to `ForwardEffect`.

- **Two-persona contract**: Dialect authors implement `Interpretable<ValueContext<'_, I>>` (and `FunctionEntry<ValueContext<'_, I>>` for callable statements) using the **inherent** `ctx.read`/`ctx.write` helpers on `ValueContext` (no helper-trait import — the helpers are inherent), value-domain bounds on `I::Value`, and — for forward rules — `I: ForwardInterp` so they can return `ForwardEffect` as `I::Effect`. Compiler authors compose language enums with derives, pick a value type, error type, engine, and linker; when needed, they can also opt into custom frame types or custom abstract policies. Imports come from `kirin_interpreter::dialect` and `kirin_interpreter::engine` respectively. Customizing traversal is part of the compiler-author surface, not a separate persona.

- **Statement dispatch**: Dialect statements implement `Interpretable<ValueContext<'_, I>>` — specialized on the forward context type; `C::Value`/`C::Error`/`C::Effect` resolve (for `C = ValueContext<'_, I>`) to `I::Value`/`I::Error`/`I::Effect`. A forward rule (`I: ForwardInterp`) reads/writes SSA state through `ValueContext`'s **inherent** helpers (`ctx.read`, `ctx.write`, `ctx.read_many`, `ctx.write_results` — which delegate to the engine's `Env` storage access; there is no `ForwardCtx` trait) and returns `Result<I::Effect, I::Error>`, building `ForwardEffect`: atomic ops return `ForwardEffect::Next`; control ops return `Jump`/`Branch` (CFG edges), `Call`, `Yield`/`Return` (completions), or `Push` (run a sub-computation by pushing a dialect-owned frame, then bind its results). There is **no** framework "scope" type and no framework "explore alternatives" effect.
