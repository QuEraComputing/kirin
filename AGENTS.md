...
**Interpreter:**
- `kirin-interpreter` — interpreter framework for concrete and abstract interpretation. `Interp` is the engine driver trait carrying associated `Value`/`Error`/`Effect`; dialect authors implement `Interpretable<C>` specialized on a **context type** `C: InterpretCtx` (forward rules: `Interpretable<ForwardContext<'_, I>>`, reading/writing via the `ForwardCtx` helpers on `ForwardContext` and returning `I::Effect`, e.g. `ForwardEffect`); compiler authors use `ConcreteInterpreter`/`AbstractInterpreter` + `Linker` components
