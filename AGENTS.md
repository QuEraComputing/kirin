...
- **Two-persona contract**: Dialect authors implement `Interpretable<I>` (and `FunctionEntry<I>` for callable statements) using only `Ctx`, the `Effect` algebra, and value-domain bounds on `I::Value`. Compiler authors compose language enums with derives, pick a value type, error type, engine, and linker — they never implement framework traits by hand. Imports come from `kirin_interpreter::dialect` and `kirin_interpreter::engine` respectively. (Customizing traversal — a custom frame type or custom abstract policies — is an opt-in extension, not a separate persona; see *Customizing traversal* below.)
...
- **Engines run frames; traversal lives in frames**: `ConcreteInterpreter<'ir, S, V, E, Lk, F = StandardFrame<V, E>>` is a generic frame-stack driver — pop the top frame, `step`, apply the returned `FrameEffect` — owning no traversal logic itself. `StandardFrame` wraps the standard `ScopeFrame`/`CallFrame`; a custom `F` (its own enum reusing them via `FrameBuild`) customizes traversal without forking the engine. `AbstractInterpreter<'ir, S, V, E, Lk, P = DefaultPolicy>` is the fixpoint analyzer over a lattice `V: Widen` (unbound reads are `bottom`): block-worklist CFG with widening, scope re-entry until stable, and interprocedural entry/return `Product<V>` summaries with caller re-enqueueing (including same-key self-recursion). Analysis crates are a lattice + a policy + an `AbstractInterpreter` type alias (see `kirin-constprop`).

- **Customizing traversal (advanced)**: the frame layer is public — `Frame<I: Interp>` (implemented by the *total* frame enum), `FrameEffect`, `FrameBuild` (embeds `ScopeFrame`/`CallFrame` into a custom enum), and `FrameDriver: Interp` (the engine capability surface frames call). Frame generics must **never** leak into `Interpretable<I>`. For abstract analyses, summary keying and join/widen are the pluggable `CallContext`/`AbstractControl` policies (one `P`, default `DefaultPolicy` = context-insensitive). `kirin-constprop`'s `ConstPropContext` is a bounded arg-tuple `CallContext` (precise recursion → `factorial(Const(5)) = Const(120)`; sound, terminating cap to `Unknown`). A future backward/liveness traversal is a new frame over `FrameDriver`, not a new `Effect`.
