...
**Interpreter:**
- `kirin-interpreter` — interpreter framework. Shared pieces: `Interp`, `Interpretable<I, Kind>`, `Frame`, and `drive_frames`. Forward pieces: `ForwardEvalInterp`, `Env`, `ForwardEffect`, `ConcreteInterpreter`, and `ForwardAbstractInterpreter`. `ForwardEval` (forward evaluation: concrete execution, const-prop, interval analysis) is the only `Kind` semantics marker implemented now; `BackwardLiveness` exists only for the coexistence test. Future sibling modes (docs-only): `ForwardType`/`ForwardTypeInterp`, `BackwardDataflow`/`BackwardDataflowInterp`, `BackwardLiveness`/`BackwardLivenessInterp`. `AbstractInterpreter` is the marker trait for lattice-valued engines.
...
- **`Interp` is the engine driver; `Interpretable<I, Kind>` is the dialect trait**: `Interp` exposes `Value`, `Error`, `Effect`, `Kind`, and the current statement location (`stage()`/`statement()`/`index()`). Dialect rules receive the engine `interp` directly and are selected by a compile-time semantics marker `Kind` (`ForwardEval` today, `BackwardLiveness` reserved), not by a runtime context object. Forward rules use `Interpretable<I, ForwardEval>`; future analyses add a new `Kind` marker instead of adding more cases to `ForwardEffect`.

- **Two-persona contract**: Dialect authors implement `Interpretable<I, ForwardEval>` (and `FunctionEntry<I>` for callable statements). A rule receives the engine `interp: &mut I` directly and uses the `interp.read`/`interp.write` helpers, which are **default methods on `ForwardEvalInterp`** operating on the engine's current activation (`interp.index()`); forward rules bound `I: ForwardEvalInterp` so they can return `ForwardEffect` as `I::Effect`, plus value-domain bounds on `I::Value`. Compiler authors compose language enums with derives, pick a value type, error type, engine, and linker; when needed, they can also opt into custom frame types or custom abstract policies. Imports come from `kirin_interpreter::dialect` and `kirin_interpreter::engine` respectively. Customizing traversal is part of the compiler-author surface, not a separate persona.

- **Statement dispatch**: Dialect statements implement `Interpretable<I, ForwardEval>` — specialized on the engine `I` and the `ForwardEval` semantics marker. A forward rule (`I: ForwardEvalInterp`) reads/writes SSA state through the `ForwardEvalInterp` **default-method** helpers (`interp.read`, `interp.write`, `interp.read_many`, `interp.write_results` — which delegate to the engine's `Env` storage access at `interp.index()`; there is no `ForwardCtx`/`ValueContext` type) and returns `Result<I::Effect, I::Error>`, building `ForwardEffect`: atomic ops return `ForwardEffect::Next`; control ops return `Jump`/`Branch` (CFG edges), `Call`, `Yield`/`Return` (completions), or `Push` (run a sub-computation by pushing a dialect-owned frame, then bind its results). There is **no** framework "scope" type and no framework "explore alternatives" effect.
...
- **Stage dispatch**: stage enums add `#[derive(InterpDispatch)]` next to `StageMeta`/`ParseDispatch`; single-language pipelines get a blanket impl. `InterpDispatch<I, Kind>` is keyed on the engine `I: Interp` and the semantics marker `Kind`. The engine sets its current location then passes itself to dispatch, which forwards to the matching `Interpretable`/`FunctionEntry` rule. Engine-internal IR queries go through `StageQuery`.
...
- **Future analyses define their own `Kind` marker + effect algebra**: for example, liveness should add a `BackwardLiveness` `Kind`, implement `Interpretable<I, BackwardLiveness>` with liveness-specific helpers on its own engine trait, its own `Effect`, and its own frame-driver capability. It should not add backward variants to `ForwardEffect`. The `Kind` marker is what lets the same dialect type carry both a forward and a backward rule without coherence conflicts. A compile-time readiness proof for this shape (both `Interpretable<I, ForwardEval>` and `Interpretable<I, BackwardLiveness>` coexisting) lives in `kirin-interpreter::dispatch::tests`.
