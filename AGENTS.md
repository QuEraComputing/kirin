...
- **`Interp` is the driver; `Interpretable<I>` is the single dialect trait**: `Interp` exposes `Value`, `Error`, and the per-statement `Effect` as **associated types**, so a dialect rule takes one generic parameter `I` instead of threading `<L, I, F, C, E, T>`. `I::Effect` is **analysis-specific**, not a universal enum: forward execution and abstract interpretation use `ForwardEffect`; a future analysis defines its own without touching the framework. Forward rules bound `I: ForwardInterp` (the flavor of `Interp` whose `Effect = ForwardEffect<I::Value, I::Frame>`). The frame type is *not* a base-`Interp` associated type — it stays the engine's own `F` generic (`ConcreteInterpreter<.., F>`); the forward flavor re-exposes it as `ForwardInterp::Frame` only so a structured dialect can name the frame it pushes in a `ForwardEffect::Push` (ordinary dialects never name it — `F` is inferred from `I::Effect`).
