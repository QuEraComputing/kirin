...
- **Engines run frames; traversal lives in frames**: `ConcreteInterpreter<'ir, S, V, E, Lk, F = StandardFrame<V, E>>` is a generic frame-stack driver — pop the top frame, `step`, apply the returned `FrameEffect` — owning no traversal logic itself. `StandardFrame` wraps the standard `ScopeFrame`/`CallFrame`; a custom `F` (its own enum reusing them via `FrameBuild`) customizes traversal without forking the engine. `AbstractInterpreter<'ir, S, V, E, Lk, P = ContextInsensitive>` is the fixpoint analyzer over a lattice `V: Widen` (unbound reads are `bottom`): block-worklist CFG with widening, scope re-entry until stable, and interprocedural entry/return `Product<V>` summaries with caller re-enqueueing (including same-key self-recursion). Analysis crates are a lattice + an analysis + an `AbstractInterpreter` type alias (see `kirin-constprop`).

- **Customizing traversal**: the frame layer is public — `Frame<I: Interp>` (implemented by the *total* frame enum), `FrameEffect`, `FrameBuild` (embeds `ScopeFrame`/`CallFrame` into a custom enum), and `FrameDriver: Interp` (the engine capability surface frames call). Frame generics must **never** leak into `Interpretable<I>`. For abstract analyses, summary keying and join/widen are the pluggable `CallContext`/`WideningStrategy` strategies (one `P`, default `ContextInsensitive`). `kirin-constprop`'s `ConstPropContext` is a bounded arg-tuple `CallContext` (precise recursion → `factorial(Const(5)) = Const(120)`; sound, terminating cap to `Unknown`). A future backward/liveness traversal is a new frame over `FrameDriver`, not a new `Effect`.
