...
use crate::node::{Block, BlockParent, Statement};
...
impl Detach for Block {
    fn detach<L: Dialect>(&self, stage: &mut StageInfo<L>) {
        let (prev, next, parent) = if let Some(info) = self.get_info_mut(stage) {
            assert!(
                !matches!(info.parent, Some(BlockParent::Statement(_))),
                "Cannot detach a block directly owned by a statement"
            );
            let prev = info.get_prev_mut().take();
            let next = info.get_next_mut().take();
            let parent = info.get_parent_mut().take();
        ...
        let Some(BlockParent::CFG(parent)) = parent else {
            return;
        };
        ...
        if prev.is_none() {
            debug_assert!(
                *parent_info.get_head() == Some(*self),
                "Parent CFG's head does not match the block being detached"
            );
            *parent_info.get_head_mut() = next;
        }
        if next.is_none() {
            debug_assert!(
                *parent_info.get_tail() == Some(*self),
                "Parent CFG's tail does not match the block being detached"
            );
            *parent_info.get_tail_mut() = prev;
        }
        *parent_info.get_len_mut() = parent_info
