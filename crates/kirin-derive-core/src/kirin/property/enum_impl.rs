...
impl<'src, S: SearchProperty> Compile<'src, Enum<'src, Property<S>>, EnumImpl> for Property<S> {
    fn compile(&self, node: &Enum<'src, Property<S>>) -> EnumImpl {
        let value_type = &self.value_type;
        let variant_ident = node.variant_names();
        let unpacking = node.unpacking();
        let glob = S::search_enum(node);
        let action = node
            .variants()
            .map(|v| {
