...
impl<'a, 'b, 'src> Compile<'src, FieldsIter, DialectStruct<'src, FieldsIter>> for StructImpl<'a, 'b, 'src> {
    fn compile(
        ctx: &'a FieldsIter,
        node: &'b DialectStruct<'src, FieldsIter>,
    ) -> syn::Result<Self> {
        Ok(StructImpl {
            src: node.input(),
            mutable: ctx.mutable,
...
pub struct StructImpl<'a, 'b, 'src> {
    ...
    fields: &'b Fields<'src, KirinFieldOptions, FieldExtra>,
    iter: IteratorImplStruct<'a, 'b, 'src>,
}

impl ToTokens for StructImpl<'_, '_ , '_> {
