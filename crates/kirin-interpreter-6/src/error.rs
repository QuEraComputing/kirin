...
pub enum InterpreterError {
    ...
    MaxDepthExceeded,
    /// Missing stage, block, or function entry.
    MissingEntry,
    /// Block argument count mismatch.
    ArityMismatch { expected: usize, got: usize },
    /// No current statement (block execution past end without a terminator).
    NoCurrent,
    /// An effect reached the driver loop with no registered handler.
    UnhandledEffect(String),
    /// Custom error from a dialect.
