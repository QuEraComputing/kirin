...
/// via the enum constructor; dialect effect/cursor E_D injects into E_L the
/// same way. NOT the same as `From` — `Lift` is semantically "coproduct
/// injection", reserved for dialect/effect/cursor composition.
pub trait Lift<From> {
...
/// through unchanged. `Err(self)` enables chained projection without allocation.
/// NOT the same as `TryFrom` — `Project` is "coproduct extraction".
