...
pub trait AbstractCallDispatch<V, C>: StageMeta + Sized {
    ...
    /// [`entry_block_of`][crate::pipeline::entry_block_of] with a fixed dialect.
    /// Multi-stage implementors match on `stage_id` and call `entry_block_of`
    /// with the appropriate dialect type.
    fn entry_block_for(
        pipeline: &Pipeline<Self>,
