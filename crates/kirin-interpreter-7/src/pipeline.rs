...
///
/// Both `ConcreteInterp` and `AbstractInterp` hold one of these, eliminating
/// the duplicated `resolve_function` / `entry_block_of` code that existed in
/// interpreter-6.
...
impl<'ir, S: StageMeta> PipelineHandle<'ir, S> {
    ...
    ///
    /// Generic over `L` so that cross-stage calls can resolve functions in a
    /// different dialect from the interpreter's home dialect.
