...
pub trait Env {
        ...
        Self::Value: ProductValue,
    {
        ...
        if results.len() == 1 {
            ...
            // Multi-result requires a product value: returning an error here avoids
            // silently dropping result slots 1..n (the iter-15 bug).
...
pub trait AbstractEnv: Env {
    ...
    ///
    /// `eval_for_abstract` passes this to `AbstractForCursor::new`. Override to
    /// change the loop widening budget. Default is 10.
