...
use kirin_interpreter_18::control::Control;
use kirin_interpreter_18::dispatch::Dispatch;
use kirin_interpreter_18::env::Env;
...
structural_error_impl!(Bind, "bind is not yet supported in interpreter18");
structural_error_impl!(
    Lambda,
...
// ---------------------------------------------------------------------------
// Call — dispatches via Dispatch::dispatch_call (no dialect-type parameter).
//
// Unlike interpreter17 where Call had a base impl that errored and dialect
// authors handled it in Interpretable<E> for MyDialect by matching on the
// Call variant, interpreter18 puts the dispatch directly on Call<T> using
// E: Dispatch. Dialect authors no longer need to match on Call in their
// Interpretable impls.
// ---------------------------------------------------------------------------
    ...
    T: CompileTimeValue,
{
    fn eval(&self, env: &mut E) -> Result<Control<E::Value, E::Ext>, E::Error> {
        ...
        env.dispatch_call(self.target(), stage, args, self.results().to_vec())
    }
}

// ---------------------------------------------------------------------------
// Lexical — delegates all arms
// ---------------------------------------------------------------------------
