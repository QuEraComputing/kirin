...
/// Summary-based interprocedural fixpoint interpreter with cursor-based SCF support.
///
/// `C` is the abstract cursor coproduct type (analogous to `C` in `ConcreteInterp`).
/// For flat CF programs without SCF, use `()` as the cursor type and the
/// abstract `BlockCursor` execute impl will handle all traversal.
    ...
    AbstractBlockCursor<V, L>: Execute<Self> + Lift<C>,
{
    ...
    fn analyze_function(&mut self, func: SpecializedFunction) -> Result<(), InterpreterError> {
        ...
        loop {
            // Process cursor stack first (for SCF cursors that need to run).
            while !self.cursor_stack.is_empty() {
                self.step_cursor(func)?;
            }

            // Then dequeue the next block from the worklist.
            let block = {
    ...
    fn step_cursor(&mut self, func: SpecializedFunction) -> Result<(), InterpreterError> {
        ...
        match effect {
            ...
            Control::Yield(v) => {
                ...
                // If no parent, the yield is lost (shouldn't happen in well-formed programs).
            }
            Control::Return(v) => {
            ...
            Control::Jump(block, args) => {
                // Shouldn't reach here: abstract BlockCursor handles Jump internally.
                // Propagate anyway for safety.
