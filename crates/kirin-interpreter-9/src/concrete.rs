    ...
    BlockCursor<V, L>: Lift<C>,
{
    pub fn step(&mut self) -> Result<bool, InterpreterError> {
        ...
        match effect {
            ...
            Control::Jump(..) => {
                // BlockCursor handles Jump internally — this is unreachable in
                // correct programs but safe to push back.
                self.cursors.push(entry);
            }
            ...
            Control::Ext(CursorExt::Pop) => {
                // Current cursor is done; discard it.
            }
            Control::Yield(v) => {
                // Deliver to parent cursor's inbox.
