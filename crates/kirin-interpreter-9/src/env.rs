...
/// Unified domain trait for interpreter-9.
///
...
///
/// Dialect semantics use `impl<E: Env> Interpretable<E> for PureOp` for
/// pure ops that work identically in both modes. SCF ops use split impls
/// bounded by `E: Env<Mode = ConcreteMode<C>>` or `AbstractMode<C>`.
pub trait Env {
    type Mode;
    type Value: Clone;
    type Ext;
...
pub trait AbstractEnv: Env {
    ...
    /// Record a return value from the current function.
