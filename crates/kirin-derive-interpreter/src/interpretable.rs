...
fn emit_interpretable(
    ...
    // semantic key `__Semantics`; the delegating wrapper impl is generic over
    // both, so one derive serves every semantics the wrapped types implement
    // (forward evaluation, strong demand, classic liveness, downstream keys,
    // ...). A deriving enum therefore cannot also hand-write a per-key
    // enum-level impl; customization belongs on the wrapped types.
    impl_generics
        .params
        .push(syn::GenericParam::Type(syn::parse_quote!(__InterpI)));
    impl_generics
        .params
        .push(syn::GenericParam::Type(syn::parse_quote!(__Semantics)));
    ...
    for stmt_ctx in ctx.statements.values() {
        if let Some(wrapper_ty) = stmt_ctx.wrapper_type {
            predicates.push(syn::parse_quote! {
                #wrapper_ty: #interp_crate::Interpretable<__InterpI, __Semantics>
            });
        }
    }
    ...
    for variant in &data.variants {
        ...
        arms.push(quote! {
            #arm_pattern => <#wrapper_ty as #interp_crate::Interpretable<__InterpI, __Semantics>>::interpret(
                #binding, interp,
            )
        });
    }
    ...
    Ok(vec![quote! {
        ...
        impl #impl_generics #interp_crate::Interpretable<__InterpI, __Semantics> for #type_name #ty_generics #where_clause {
            fn interpret(
                &self,
