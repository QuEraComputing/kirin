...
use kirin_interpreter_15::algebra::Lift;
...
use crate::interpreter15::cursor::{AbstractForCursor, AbstractIfCursor, ForCursor, IfCursor};
    ...
    T: CompileTimeValue,
{
    ...
    let carried = E::Value::new_product(init_values);
    let body_stage = env.current_stage();
    let max_iter = env.for_widening_budget();
...
// ---------------------------------------------------------------------------
// NOTE: StructuredControlFlow does NOT get a blanket Interpretable<E> impl.
//
// In iter-13, the blanket impl existed but returned a runtime Err for If/For
// variants, creating false compile-time safety. Removing it forces dialect
// authors to implement `Interpretable` only for concrete enum types (e.g.,
// via `#[wraps]` composition) and explicitly route If/For through ScfSeam.
// Any attempt to call .eval() on a raw StructuredControlFlow will now fail
// at compile time rather than panicking at runtime.
// ---------------------------------------------------------------------------
