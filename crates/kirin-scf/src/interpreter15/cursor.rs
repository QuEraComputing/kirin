...
//
// Key iter-14 improvement: WaitBody phase stores `prev_carried` so we can
// check `is_subseteq` for early convergence before the budget is exhausted.
// ---------------------------------------------------------------------------
    ...
    fn execute(
        ...
        match std::mem::replace(&mut self.phase, AbstractForPhase::Done(PhantomData)) {
            ...
            AbstractForPhase::WaitBody {
                ...
                // Early convergence: stop if new_carried ⊑ prev_carried.
                ...
                if iterations >= self.max_iterations {
                    // Widening: join to ensure monotone termination.
