...
use kirin_interpreter_10::env::{AbstractEnv, AbstractMode, ConcreteMode, Env};
...
use crate::interpreter10::cursor::{AbstractForCursor, AbstractIfCursor, ForCursor, IfCursor};
...
where
    ...
    // Fixed: AbstractBlockCursor is what AbstractIfCursor needs to lift.
...
where
    ...
    E: Env<Mode = AbstractMode<C>>,
    E::Value: Clone + ProductValue + AbstractValue + 'static,
    // Fixed: AbstractBlockCursor is what AbstractForCursor needs to lift.
    AbstractBlockCursor<E::Value, L>: Lift<C>,
        ...
        match self {
            ...
            StructuredControlFlow::If(_) | StructuredControlFlow::For(_) => {
                Err(E::Error::from(InterpreterError::UnhandledEffect(
                    "scf.if and scf.for must be handled by the composed language's Interpretable \
                     impl (call eval_if_concrete/abstract or eval_for_concrete/abstract)"
                        .into(),
                )))
            }
        }
