...
use kirin_interpreter::dialect::{
    BranchCondition, Ctx, Effect, EnvOps, Interp, Interpretable, InterpreterError, Scope,
    ScopeHook, ScopeStep,
};
    ...
    T: CompileTimeValue,
{
    fn interpret(&self, ctx: &mut Ctx<'_, I>) -> Result<Effect<I::Value, I::Error>, I::Error> {
        ...
        match ctx.read(self.condition)?.is_truthy() {
            Some(true) => Ok(Effect::Enter(then_scope())),
            Some(false) => Ok(Effect::Enter(else_scope())),
            None => Ok(Effect::EnterAny(vec![then_scope(), else_scope()])),
        }
    }
}
...
{
    fn interpret(&self, ctx: &mut Ctx<'_, I>) -> Result<Effect<I::Value, I::Error>, I::Error> {
        Ok(Effect::Yield(ctx.read_many(self.values.as_slice())?))
    }
}
