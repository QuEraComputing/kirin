...
///
/// `S` — stage container type.
/// `L` — the dialect (or composed language type).
/// `V` — value type.
/// `C` — cursor coproduct type.
    ...
    C: Execute<Self> + 'static,
{
    ...
    where
        S: HasStageInfo<LD>,
        BlockCursor<V, LD>: Execute<Self> + Lift<C> + 'static,
    {
    ...
    BlockCursor<V, L>: Lift<C>,
{
    ...
    pub fn step(&mut self) -> Result<bool, InterpreterError> {
        ...
        let effect: Control<V, CursorExt<C>> = cursor.execute(self)?;
        ...
        match effect {
            ...
            Control::Jump(..) => {
                // BlockCursor handles Jump internally — reaching here indicates
                // a malformed program.
                self.cursors.push(cursor);
            }
            Control::Ext(CursorExt::Push(new_cursor)) => {
                // Push the current cursor back, then push the new one on top.
                self.cursors.push(cursor);
