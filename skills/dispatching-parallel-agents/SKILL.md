...
description: Use when 2+ independent problems need concurrent investigation or fixing — multiple test failures across different files, independent subsystem bugs, or parallel research tasks with no shared state or sequential dependencies.
---
...
**When agents EDIT code, use `isolation: "worktree"` on each Agent call.**

Without worktree isolation, concurrent editing agents cause:
- **Cascading build errors** — Agent A renames a file while Agent B imports from the old path
- **LSP diagnostic floods** — Incomplete intermediate states trigger spurious errors
- **Linter interference** — Auto-formatters revert one agent's edits while another is mid-change

| Agent type | Isolation needed? |
|-----------|------------------|
| Read-only (research, review, planning) | No |
