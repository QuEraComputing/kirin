...
**R7 mandatory pre-check — vestigial trait audit (run before scoring R7):** Indirection through intermediate traits is only justified when it reduces total impl count *or* provides genuine behavioral variation across its implementors. Before scoring R7, apply the following two metrics to every non-object-safe trait defined in the interpreter or dialect crates:

1. **Behavioral distinctness** — For each method on the trait, count the number of implementations whose bodies are *pure delegation* (`self.field.method(args)` or equivalent with no added logic). If every implementation is pure delegation (distinctness = 0%), the trait provides no customization value and is vestigial.

2. **Net impl economy** — Count total impl blocks that would exist *without* the trait (one per (env type × op type) pair the blanket covers), minus total items needed *with* the trait (trait definition + blanket impls + per-env impls of the trait). If net economy ≤ 0, the trait costs as much as it saves.

A trait is load-bearing when: net economy > 0 **and** behavioral distinctness > 0 (at least one implementor does something meaningfully different from delegation). A trait that fails both tests should be flagged as a finding under R7. A trait that fails one test at the margin (e.g. saves 1–2 impl blocks but all impls delegate) is a weak justification — note it and score R7 accordingly.

**Example of a vestigial trait (calibration):** A `ScfSeam<T>` trait with blanket impls for `ConcreteInterp`/`AbstractInterp` (these do real work) and delegation impls for user newtypes (`self.0.eval_if(op)`) has partial behavioral distinctness. Its net economy is +1 (saves one impl block total across all env types). This marginal result does not justify a finding by itself, but a trait where *all* impls delegate and economy is 0 or negative is a clear R7 finding.
...
| R7 | **Algebraic elegance** | Lift/Project, Mode, Cursor, and Env form a coherent algebra; naming is consistent; a new developer can predict the pattern from one example; all non-object-safe traits pass the vestigial trait pre-check (behavioral distinctness > 0, net impl economy > 0) | Mostly coherent; some naming inconsistencies or ad-hoc special cases; one or two traits are marginally justified (economy = 1, all impls delegate) | Ad-hoc design; each new case requires a novel pattern; or one or more traits have economy ≤ 0 and zero behavioral distinctness |
...
| R10 | **Implementation readability** | Each file has a single clear responsibility and stays under ~150 lines; module hierarchy mirrors the conceptual hierarchy; type names read naturally when module-qualified with no redundancy; single-letter type params only where unambiguous by convention; no expression pattern repeated ≥ 5 times without a shorthand; no intra-expression naming redundancy; no constructor chain requiring ≥ 4 wrapping levels to express one intent | Most files are focused; one or two exceed ~200 lines or mix concerns; qualified names mostly read well; one repeated expression pattern (< 10 occurrences) without a shorthand | Implementation in one or two giant files (500+ lines); or a multi-level expression pattern with ≥ 10 occurrences and no shorthand; or systematic intra-expression name redundancy |

**R10 mandatory pre-check — expression boilerplate audit (run before scoring R10):** File-level metrics (line count, module structure) miss expression-level readability problems. Before scoring R10, check the following:

1. **Repetition threshold** — Search for any multi-level constructor expression (nesting depth ≥ 3) that appears 4 or more times across the codebase. Each such pattern is a candidate for a constructor shorthand method. Count total occurrences; flag any pattern appearing ≥ 5 times as a finding. Example: `Ok(Control::Ext(CursorExt::Push(x.lift())))` appearing 113 times across 42 files is a critical R10 violation — a method `Control::push(cursor)` would eliminate the wrapping noise at every call site.

2. **Intra-expression DRY violation** — Scan constructor chains for the same concept expressed more than once within one expression. This is a DRY violation at the expression level: the programmer encodes the same information twice, which means a change to one requires a change to the other. Example: `Control::Ext(CursorExt::Push(...))` — "Ext" appears in both the `Control` variant name and the `CursorExt` type name; the concept "this is the extension/cursor slot" is stated twice with no new information gained. The correct fix is to eliminate the duplication structurally, either by renaming so each token is distinct, or by collapsing the two levels into one (`Control::push(cursor)` encodes the concept once).
