...
last_updated = "2026-02-11T05:33:51.340219Z"
dependencies = ["0001"]
+++
...
Define a two-dialect split inside `kirin-function` for closure and call semantics:
`Lexical` (lambda, call, ret) and `Lifted` (bind, call, ret).
...
- `dialects/bind.rs`:
  - `bind` for constructing callable values from a target symbol and captures.
- `dialects/call.rs`:
  - direct function call operation (`call`).
- `dialects/ret.rs`:
  - return terminator (`ret`).
- `dialects/mod.rs`:
  - `Lexical<T>` enum (FunctionBody, Lambda, Call, Return) for pre-lifting IR.
  - `Lifted<T>` enum (FunctionBody, Bind, Call, Return) for post-lifting IR.
...
| --- | --- | --- |
| `kirin-function` | two dialect enums `Lexical` (lambda, call, ret) and `Lifted` (bind, call, ret) with individual operation modules | unit tests for each operation + composition tests |
...
- Users must choose the correct dialect enum (`Lexical` vs `Lifted`) for their
  compilation stage.
...
- Migration steps:
  1. Introduce `Lexical<T>` and `Lifted<T>` dialect enums in `kirin-function`.
  2. Add parser/printer support and examples for `lambda`, `bind`, `call`, `ret`.
...
- Teach operation families:
  - `Lexical<T>` dialect: `lambda` creates callable syntax-level closures, `call` invokes known symbols, `ret` terminates
  - `Lifted<T>` dialect: `bind` constructs callable objects from lifted closures, `call` invokes known symbols, `ret` terminates
- Update:
...
2. Add statement definitions for `lambda`, `bind`, `call`, and `ret`.
...
- [ ] `kirin-function` exposes `Lexical<T>` and `Lifted<T>` dialect enums
      with separate modules for lambda, bind, call, and return operations.
...
- Follow-up tasks:
  - evaluate whether `call` should eventually carry effect/speculation
    attributes
...
- Add indirect call / `apply` operation in future if needed for dynamic dispatch.
...
| 2026-02-11 | Split into `Lexical<T>` / `Lifted<T>` two-dialect design; removed `Apply` and `Callable` wrapper |
