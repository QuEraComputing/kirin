...
use kirin_function::interpreter15::interpret::{CallSeam, eval_call_for_dialect};
use kirin_interpreter::{AbstractValue, BranchCondition, ProductValue};
use kirin_interpreter_15::abstract_call_dispatch::AbstractCallDispatch;
use kirin_interpreter_15::abstract_interp::AbstractInterp;
use kirin_interpreter_15::algebra::{Lift, Project, SingleStageCursorFor};
use kirin_interpreter_15::call_dispatch::CallDispatch;
use kirin_interpreter_15::concrete::ConcreteInterp;
use kirin_interpreter_15::control::{Control, CursorExt};
use kirin_interpreter_15::cursor::{AbstractBlockCursor, BlockCursor};
use kirin_interpreter_15::env::{AbstractEnv, AbstractMode, ConcreteMode, Env};
use kirin_interpreter_15::error::InterpreterError;
use kirin_interpreter_15::execute::Execute;
use kirin_interpreter_15::interpretable::Interpretable;
use kirin_interpreter_15::pipeline::PipelineHandle;
use kirin_scf::ForLoopValue;
use kirin_scf::interpreter15::cursor::{
    AbstractForCursor, AbstractIfCursor, AbstractSCFCursor, ForCursor, IfCursor, SCFCursor,
};
use kirin_scf::interpreter15::interpret::ScfSeam;
...
//
// Iteration-15: associated-type bounds folded into ToyVal's supertrait list.
// Callers write only `V: ToyVal` — the two previously repeated where-clause
// lines are now derived from the supertrait definitions, eliminating them from
// every Execute and Interpretable impl.
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    use kirin_interpreter_15::cursor::AbstractBlockCursor;

    use crate::interpreter15::{
...
"#;
    ...
    fn for_loop_sum_zero_iterations() {
        // sum_range(0): loop never executes, result is initial accumulator = 0
    ...
    fn for_loop_abstract_converges() {
        // Abstract interpretation of sum_range with ToyType lattice.
        // The loop body: new_acc = acc + iv. With IV = Bottom (abstract) and
        // acc = I64, the body yields I64. Since I64 ⊑ I64, convergence is
        // immediate — the AbstractForCursor exits after one iteration.
