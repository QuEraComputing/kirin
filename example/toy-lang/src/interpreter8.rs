...
use kirin_scf::interpreter8::cursor::{ForCursor, IfCursor, SCFCursor};
...
// Execute<D> for HighLevelCursor: dispatch to inner cursor.
// TODO: #[derive(ComposedCursor)] generates this.
impl<D, V> Execute<D> for HighLevelCursor<V>
where
    V: Clone
        + BranchCondition
        + ForLoopValue
    ...
    D: ConcreteEnv<Value = V, Cursor = HighLevelCursor<V>>,
    D::Ext: From<CursorExt<HighLevelCursor<V>>>,
    D::Stages: HasStageInfo<HighLevel>,
    D::Error: From<InterpreterError>,
    HighLevel: Semantics<D, Effect = Control<V, D::Ext>>,
{
    fn execute(&mut self, domain: &mut D) -> Result<Control<V, D::Ext>, D::Error> {
        match self {
            HighLevelCursor::Block(c) => c.execute(domain),
            HighLevelCursor::Scf(c) => c.execute(domain),
        }
    }
}

// ---------------------------------------------------------------------------
// HighLevel: Semantics<ConcreteInterp<...>>
// ---------------------------------------------------------------------------

impl<'ir, V> Semantics<ConcreteInterp<'ir, Stage, HighLevel, V, HighLevelCursor<V>>> for HighLevel
where
    V: Clone
        + BranchCondition
        + ForLoopValue
    ...
    <V as CompareValue>::Bool: Into<V>,
{
    type Effect = Control<V, CursorExt<HighLevelCursor<V>>>;

    fn eval(
        &self,
        domain: &mut ConcreteInterp<'ir, Stage, HighLevel, V, HighLevelCursor<V>>,
    ) -> Result<Control<V, CursorExt<HighLevelCursor<V>>>, InterpreterError> {
        match self {
            HighLevel::Lexical(op) => op.eval(domain),
            HighLevel::Structured(op) => op.eval(domain),
            HighLevel::Constant(op) => op.eval(domain).map(Control::from),
            HighLevel::Arith(op) => op.eval(domain).map(Control::from),
            HighLevel::Cmp(op) => op.eval(domain).map(Control::from),
            HighLevel::Bitwise(op) => op.eval(domain).map(Control::from),
        }
    }
}

// ---------------------------------------------------------------------------
// LowLevel: Semantics — concrete and abstract modes
// ---------------------------------------------------------------------------

impl<'ir, V, C> Semantics<ConcreteInterp<'ir, Stage, LowLevel, V, C>> for LowLevel
where
    V: Clone
        + BranchCondition
        + ProductValue
    ...
    C: 'static,
{
    ...
    fn eval(
        &self,
        domain: &mut ConcreteInterp<'ir, Stage, LowLevel, V, C>,
    ) -> Result<Control<V, CursorExt<C>>, InterpreterError> {
        match self {
            LowLevel::Lifted(op) => op.eval(domain),
            LowLevel::Constant(op) => op.eval(domain).map(Control::from),
            LowLevel::Arith(op) => op.eval(domain).map(Control::from),
            LowLevel::Cmp(op) => op.eval(domain).map(Control::from),
            LowLevel::Bitwise(op) => op.eval(domain).map(Control::from),
            LowLevel::Cf(op) => op.eval(domain),
        }
    }
}

impl<'ir, V> Semantics<AbstractInterp<'ir, Stage, LowLevel, V>> for LowLevel
where
    V: Clone
        + AbstractValue
        + BranchCondition
    ...
    <V as CompareValue>::Bool: Into<V>,
{
    ...
    fn eval(
        &self,
        domain: &mut AbstractInterp<'ir, Stage, LowLevel, V>,
...
mod tests {
    ...
    use crate::interpreter8::HighLevelCursor;
    use crate::language::{HighLevel, LowLevel};
    use crate::stage::Stage;
    ...
    where
        ...
        HighLevel: Semantics<
                ...
                Effect = Control<V, CursorExt<HighLevelCursor<V>>>,
            >,
        HighLevelCursor<V>:
            Execute<ConcreteInterp<'static, Stage, HighLevel, V, HighLevelCursor<V>>>,
        BlockCursor<V, HighLevel>: Lift<HighLevelCursor<V>>,
    {
