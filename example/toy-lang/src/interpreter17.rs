...
use kirin_interpreter_17::env::{AbstractEnv, AbstractMode, ConcreteMode, Env};
use kirin_interpreter_17::error::InterpreterError;
use kirin_interpreter_17::execute::Execute;
use kirin_interpreter_17::interpretable::Interpretable;
use kirin_interpreter_17::pipeline::PipelineHandle;
...
"#;
    ...
    // Uses `BackwardFixpoint` from `kirin-interpreter-17` for the fixpoint
    ...
    use kirin_interpreter_17::backward::{BackwardFixpoint, BlockTransferBackward};
    ...
    // -----------------------------------------------------------------------
    // Statement-level backward liveness helpers (iter-17)
    // -----------------------------------------------------------------------

    /// Collect SSAValues used in `block` that are not locally defined (free variables).
    /// Recurses into nested blocks via stmt.blocks(stage).
    ...
    /// Compute statement-level backward liveness for a single-block function.
    /// Returns a map from each Statement to the set of SSAValues live BEFORE that statement.
    ...
    /// Backward liveness on a HighLevel single-block function with scf.if.
    ///
    /// ABS_SOURCE: ^entry(%x) { %zero=const0; %is_neg=lt %x,%zero; %result=if %is_neg {...} -> i64; ret %result }
    ///
    /// Statement-level backward liveness at the scf.if:
    ///   - %is_neg is the condition (direct argument to if)
    ///   - ^neg uses %x as free var; ^pos uses %x as free var
    ///   - live_before_if = {%is_neg, %x}
    #[test]
    fn backward_liveness_highlevel() {
        ...
        // Collect statements: [constant 0, lt, if (scf.if), ...], terminator = ret
        let stmts: Vec<Statement> = top_block.statements(stage_info).collect();
        // stmts[0] = constant 0 (%zero)
        // stmts[1] = lt %x, %zero -> %is_neg
        // stmts[2] = scf.if %is_neg ...
        assert!(
            stmts.len() >= 3,
    ...
    /// Backward liveness on FACTORIAL_SOURCE with scf.if and recursive call.
    ///
    /// At the scf.if:
    ///   - condition uses %is_base
    ///   - ^base block uses %one as free var (yield %one)
    ///   - ^recurse block uses %n and %one as free vars
    ///   - live_before_if = {%is_base, %n, %one}
    #[test]
    fn backward_liveness_scf() {
        ...
        // Statements: [constant 1 (%one), le %n,%one->%is_base, scf.if %is_base {...}]
    ...
    // -----------------------------------------------------------------------
    // Sparse abstract interpretation tests (iter-17 key feature)
    //
    // Sparse AI: pass Interval::bottom() / ToyType::Bottom for unused args.
    // The framework must handle bottom-valued reads gracefully (return bottom,
    // not error). With kirin-interpreter-17, AbstractInterp::read() returns
    // V::bottom() for absent SSA values instead of Err(UnboundValue).
    // -----------------------------------------------------------------------

    #[test]
    fn sparse_interval_propagation() {
        // SPARSE_PROG: %result = add %a, %b; %c unused
        // Seed %a=[1,3], %b=[2,4], %c=bottom (sparse/uninitialized)
        // Expected: %result = [3,7]; %c never read so bottom doesn't affect result
    ...
    fn sparse_type_propagation() {
        // Same SPARSE_PROG: %c is Bottom (sparse/uninitialized)
        // Seed %a=I64, %b=I64, %c=Bottom
        // Expected: %result = join(I64, I64) = I64
        let pipeline = build_pipeline(SPARSE_PROG);
