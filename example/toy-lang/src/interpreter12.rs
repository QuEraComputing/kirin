...
use kirin_function::interpreter12::interpret::{CallSeam, eval_call_for_dialect};
use kirin_interpreter::{AbstractValue, BranchCondition, ProductValue};
use kirin_interpreter_12::abstract_call_dispatch::AbstractCallDispatch;
use kirin_interpreter_12::abstract_interp::AbstractInterp;
use kirin_interpreter_12::algebra::Lift;
use kirin_interpreter_12::call_dispatch::CallDispatch;
use kirin_interpreter_12::concrete::ConcreteInterp;
use kirin_interpreter_12::control::{Control, CursorExt};
use kirin_interpreter_12::cursor::{AbstractBlockCursor, BlockCursor};
use kirin_interpreter_12::env::{AbstractMode, ConcreteMode, Env};
use kirin_interpreter_12::error::InterpreterError;
use kirin_interpreter_12::execute::Execute;
use kirin_interpreter_12::interpretable::Interpretable;
use kirin_interpreter_12::pipeline::PipelineHandle;
use kirin_scf::ForLoopValue;
use kirin_scf::interpreter12::cursor::{
    AbstractForCursor, AbstractIfCursor, AbstractSCFCursor, ForCursor, IfCursor, SCFCursor,
};
use kirin_scf::interpreter12::interpret::ScfSeam;
...
//
// Iteration-12: same supertrait as iteration-11, but without the '+ 'static
// bound. The concrete/abstract interpreters in interpreter-12 don't require
// 'static on their cursor coproducts; Lift uses only IR handles (Copy types).
// ---------------------------------------------------------------------------
...
where
    ...
    <V as TryFrom<ArithValue>>::Error: std::error::Error + Send + Sync + 'static,
    <V as CompareValue>::Bool: Into<V>,
    E: kirin_interpreter_12::env::AbstractEnv<
            Value = V,
...
//
// Iteration-12 key improvement: instead of 4 monomorphized impls (concrete
// single-stage, abstract single-stage, concrete multi-stage, abstract
// multi-stage), there is ONE generic impl that delegates SCF and Call
// operations through seam traits. The seam traits have blanket impls for
// ConcreteInterp and AbstractInterp, so callers get the right behavior
// based on the interpreter type at the call site.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// LowLevel: Interpretable — single generic impl (unchanged from iter-11)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// CallSeam<HighLevel> — 4 specific impls (one per interpreter × mode)
//
// These are required because the single-stage and multi-stage impls need
// different call semantics (multi-stage must try cross-stage fallback),
// and Rust blanket impls would conflict with specific impls at the same type.
// ---------------------------------------------------------------------------

// Single-stage concrete: delegates to eval_call_for_dialect.
impl<'ir, V> CallSeam<HighLevel> for ConcreteInterp<'ir, Stage, HighLevel, V, HighLevelCursor<V>>
where
    V: ToyVal,
    <V as TryFrom<ArithValue>>::Error: std::error::Error + Send + Sync + 'static,
    <V as CompareValue>::Bool: Into<V>,
{
    fn eval_call(
        ...
        eval_call_for_dialect::<_, HighLevel, _>(op, self)
    }
}

// Single-stage abstract: delegates to eval_call_for_dialect.
impl<'ir, V> CallSeam<HighLevel>
    for AbstractInterp<'ir, Stage, HighLevel, V, HighLevelAbstractCursor<V>>
where
    V: AbstractToyVal,
    <V as TryFrom<ArithValue>>::Error: std::error::Error + Send + Sync + 'static,
    <V as CompareValue>::Bool: Into<V>,
...
where
    ...
    <V as TryFrom<ArithValue>>::Error: std::error::Error + Send + Sync + 'static,
    <V as CompareValue>::Bool: Into<V>,
    E: kirin_interpreter_12::env::AbstractEnv<Value = V, Ext = CursorExt<AbstractMultiCursor<V>>>,
...
mod tests {
    ...
    use kirin_interpreter_12::cursor::AbstractBlockCursor;

    use crate::interpreter12::{
...
"#;
    ...
    // -----------------------------------------------------------------------
    // ConstProp — extensibility probe (iteration-12)
    //
    // Carried over from iteration-11. Demonstrates that the interpreter
    // framework is extensible: a new analysis domain can be added entirely
    // within toy-lang without any framework changes.
    // -----------------------------------------------------------------------
    ...
    //
    // Implemented entirely in toy-lang — no changes to any interpreter crate
    // or dialect crate. Proves the framework is extensible.
    // -----------------------------------------------------------------------
